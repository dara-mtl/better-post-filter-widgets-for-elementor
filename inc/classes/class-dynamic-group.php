...
class BPF_Dynamic_Group {
	...
	public static function add_controls_section($element) {
		$element->start_controls_section('dynamic_section',
			[
		...
		$repeater->add_control('dynamic_fields', [
		...
		$repeater->add_control('dynamic_type', [
		...
		$repeater->add_control( 'data_attr_note', [
			'type' => \Elementor\Controls_Manager::RAW_HTML,
			'raw' => __('<div class="elementor-control-field-description" style="margin:0">Set custom attributes for the wrapper element. Each attribute in a separate line. Separate attribute key from the value using Key|Value.</div>', 'bpf-widget'),
			'condition' => [
				'dynamic_type' => 'data-attribute',
			],
		]);
		
		$repeater->add_control('dynamic_target', [
