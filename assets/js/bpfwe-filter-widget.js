( function ( $ ) {
	...
	$( window ).on( 'elementor/frontend/init', function () {
		...
		// Determine if the script needs to be attached the filter widget or the search bar.
		...
		const FilterWidgetHandler = elementorModules.frontend.handlers.Base.extend( {
			...
			getAjaxFilter () {
				...
				let groupLogic = filterSetting.group_logic,
					...
					nothing_found_message = filterSetting.nothing_found_message,
					currentPage = 1,
					paginationType = '';
				...
				function get_form_values ( paged ) {
					...
					$( '.bpfwe-taxonomy-wrapper input:checked, .bpfwe-custom-field-wrapper input:checked' ).each(
						function () {
							var self = $( this );
					...
					$( '.bpfwe-custom-field-wrapper input.input-text' ).each(
						function () {
							var self = $( this );
							if ( self.val() ) {
					...
					$( '.bpfwe-taxonomy-wrapper select option:selected, .bpfwe-custom-field-wrapper select option:selected' ).each(
						function () {
							var self = $( this );
							if ( self.val() ) {
								var targetArray = self.closest( '.bpfwe-taxonomy-wrapper' ).length ? category : custom_field;
					...
					$( '.bpfwe-numeric-wrapper input' ).each(
						function () {
							var self = $( this );
							...
							if ( self.val() === '' || self.val() != initial_val ) {
								...
								$( '.bpfwe-numeric-wrapper' ).find( 'input' ).each(
									function () {
										var _this = $( this );
					...
					$.ajax( {
						...
						success: function ( data ) {
							...
							if ( data === '0' || currentFormState === originalFormState ) {
								targetSelector.off();
