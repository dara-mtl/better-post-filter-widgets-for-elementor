(function ($) {
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        // Determine if the script needs to be attached to the filter widget or the search bar.
        ...
        const FilterWidgetHandler = elementorModules.frontend.handlers.Base.extend({
            ...
            getAjaxFilter() {
                ...
                $(document).on('click', '.low-group-trigger', function () {
                    var $trigger = $(this);
                    var $lowTermsGroup = $trigger.closest('li').next('.low-terms-group');
                ...
                if (!filterSetting) { // Prevent script from running on backend.
                    return;
                }

                // Fetch the targeted post widget.
                ...
                if (!targetPostWidget || !$(targetPostWidget).length) {
                    ...
                    if ($closestWidget.length) {
                        ...
                        targetPostWidget = $closestWidget.attr('id')
                            ? `#${$closestWidget.attr('id')}`
                            : widgetClass ? `.${widgetClass}` : '';
                    }
                }

                if (!targetPostWidget || targetPostWidget === '.') {
                    return;
                }
                ...
                var maxPage = pagination.data('max-page'),
                    filterWidgetObservers = filterWidgetObservers || {};

                // Fetch the targeted post settings.
                ...
                filterWidget.on(
                    ...
                    debounce(
                        function (e) {
                            var formID = filterWidget.closest('.elementor-widget-filter-widget').data('id');

                            var formSettings = filterWidget.closest('.elementor-widget-filter-widget').data('settings');
                            if (!isSubmitPresent && formSettings.target_selector) {
								targetSelector = $(formSettings.target_selector);
                                if (e.type === 'change' || (e.type === 'keydown' && e.key === 'Enter')) {
                                    ...
                                    get_form_values(targetSelector, filterWidget, formID, formSettings);
                                    return;
                                }
                ...
                filterWidget.on(
                    ...
                    function () {
						...
						if (formSettings.target_selector) {
							targetSelector = $(formSettings.target_selector);
							resetURL();
							targetSelector.addClass('filter-initialized');
							targetSelector.removeClass('filter-active');
                ...
                $(document).on(
                    ...
                    'form.form-order-by',
                    function () {
                        var formID = $(this).closest('.elementor-widget-sorting-widget').data('id');
                        var formSettings = $(this).closest('.elementor-widget-sorting-widget').data('settings');
                        if (!formSettings || !formSettings.target_selector || !$(formSettings.target_selector).length) {
                            return;
                        }
                        targetSelector = $(formSettings.target_selector);
                        targetSelector.addClass('filter-initialized');
                ...
                $(document).off('submit', 'form.search-post').on(
                    ...
                    'form.search-post',
                    function () {
                        var formID = $(this).closest('.elementor-widget-search-bar-widget').data('id');
                        var formSettings = $(this).closest('.elementor-widget-search-bar-widget').data('settings');
                        if (!formSettings || !formSettings.target_selector || !$(formSettings.target_selector).length) {
                            return;
                        }
                        targetSelector = $(formSettings.target_selector);
                ...
                if (currentUrl.includes('?search=')) {
                    get_form_values(targetSelector, filterWidget);
                }
                ...
                $(document).on(
                    ...
                    function (e) {
                        ...
                        get_form_values(targetSelector, filterWidget, null, filterSetting, paged);
                    }
                );
                ...
                function get_form_values(targetSelector, filterWidget, formID, formSettings, paged) {
                    var postContainer = targetSelector.find('.post-container'),
                        order = '',
                        order_by = '',
                        order_by_meta = '';
					
                    // Fallback to filterSetting if formSettings is undefined (e.g., initial load or pagination).
                    formSettings = formSettings || filterSetting;

                    if (!formSettings || !formSettings.target_selector || !$(formSettings.target_selector).length) {
                        return;
                    }

					const target = formSettings.target_selector;

					// Sorting.
					$('.elementor-widget-sorting-widget').each(function () {
						const settings = $(this).data('settings') || {};
						if (settings.target_selector === target) {
							$(this).find('.form-order-by select option:selected').each(function () {
								var self = $(this);
					...
					// Search.
					var searchQuery = '';
					$('.elementor-widget-search-bar-widget').each(function () {
						const settings = $(this).data('settings') || {};
						if (settings.target_selector === target) {
							const query = $(this).find('form.search-post input').val();
					...
					let active_post_type = formSettings.filter_post_type || formSettings.post_type || 'post';
					if (filterWidget.length && filterSetting && filterSetting.target_selector === target && filterSetting.filter_post_type) {
						active_post_type = filterSetting.filter_post_type;
					}
                    ...
                    // Collect filters only from the current filterWidget.
                    filterWidget.find('.bpfwe-taxonomy-wrapper input:checked, .bpfwe-custom-field-wrapper input:checked').each(
                        function () {
                            var self = $(this);
                    ...
                    filterWidget.find('.bpfwe-custom-field-wrapper input.input-text').each(
                        function () {
                            var self = $(this);
                            if (self.val()) {
                    ...
                    filterWidget.find('.bpfwe-taxonomy-wrapper select option:selected, .bpfwe-custom-field-wrapper select option:selected').each(
                        function () {
                            var self = $(this);
                            if (self.val()) {
                                var targetArray = self.closest('.bpfwe-taxonomy-wrapper').length ? category : custom_field;
                    ...
                    filterWidget.find('.bpfwe-numeric-wrapper input').each(
                        function () {
                            var self = $(this);
                            ...
                            if (self.val() === '' || self.val() != initial_val) {
                                ...
                                filterWidget.find('.bpfwe-numeric-wrapper').find('input').each(
                                    function () {
                                        var _this = $(this);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
							...
							$('.elementor-widget-sorting-widget').each(function () {
								const settings = $(this).data('settings') || {};
								if (settings.target_selector === targetSelector.data('settings')?.target_selector) {
									firstOptionSelected = $(this).find('.form-order-by select option:first').prop('selected');
									return false;
								}
							});

							if (data === '0' || (currentFormState === originalFormState && firstOptionSelected && !searchQuery)) {
                                targetSelector.off();
                                ...
                                if (!$(content).text().trim()) {
                                    ...
                                    if (nothing_found_message && nothing_found_message.trim().length) {
                                        targetSelector.html('<div class="no-post">' + nothing_found_message + '</div>');
                                    }
                                } else {
                                    ...
                                    var loadMoreButton = targetSelector.find('.load-more'),
                                        elementorLoadMoreButton = targetSelector.find('.e-load-more-anchor').nextAll().find('.elementor-button-link.elementor-button');
                        ...
                        complete: function () {
                            ...
                            if (targetSelector.hasClass('filter-active') && paginationType == 'infinite') {
                                debounce(
                                    function (e) {
                                        bpfwe_infinite_scroll(widgetID, targetSelector);
                                    },
                                    800
                                )();
                            }
                                        ...
                                        function (entry) {
                                            if (entry.isIntersecting) {
                                                if (!ajaxInProgress && targetSelector.hasClass('filter-active')) {
                                                    ajaxInProgress = true;
                                                    currentPage++;
                                                    get_form_values(targetSelector, filterWidget, null, filterSetting, currentPage);
                                                }
                                            }
                                        }
                                    );
				...
				filterWidget.on(
					...
					'.reset-form',
					function () {
						var resetButtonForm = filterWidget.closest('.elementor-widget-filter-widget');
						var formSettings = resetButtonForm.data('settings');
						var resetTargetSelector = formSettings.target_selector;

						$(document).find('form.form-order-by').each(function () {
							var $sortingWidget = $(this).closest('.elementor-widget-sorting-widget');
							var settings = $sortingWidget.data('settings') || {};
							var formTargetSelector = settings.target_selector;
							if (formTargetSelector === resetTargetSelector) {
								$(this).find('select').prop('selectedIndex', 0).trigger('change');
							}
						});

						$(document).find('form.search-post').each(function () {
							var $searchWidget = $(this).closest('.elementor-widget-search-bar-widget');
							var settings = $searchWidget.data('settings') || {};
							var formTargetSelector = settings.target_selector;
							if (formTargetSelector === resetTargetSelector) {
								$(this).find('input[name="s"]').val('').trigger('input');
							}
						});

						var formID = resetButtonForm.data('id');
						resetURL();
						targetSelector.addClass('filter-initialized');
						targetSelector.removeClass('filter-active');
						get_form_values(targetSelector, filterWidget, formID, formSettings);
